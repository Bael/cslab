...
public class LongestIncSequence {
    ...
    public static void main(String[] args) {
        ...
        LongestIncSequence seq = new LongestIncSequence();
//        BiFunction<Long, Long, Boolean> cmpFunction = (aLongLeft, aLongRight) -> aLongLeft >= aLongRight && (aLongRight % aLongLeft == 0);
        ...
        BiFunction<Long, Long, Boolean> cmpFunction = (aLongLeft, aLongRight) -> aLongLeft <= aLongRight;
        int[] result = seq.find(numbers, cmpFunction);
    ...
    private int[] find(long[] numbers, BiFunction<Long, Long, Boolean> orderFunction) {
        ...
        for (int i = 0; i < numbers.length; i++) {
            countSeq[i] = 1;
            for (int j = 0; j < i; j++) {
                if (countSeq[j] + 1 > countSeq[i] && orderFunction.apply(numbers[j], numbers[i])) {
                    countSeq[i] = countSeq[j] + 1;
                }
            }
        }
        return restoreSeq(numbers, countSeq, orderFunction);
    }

    private int[] restoreSeq(long[] numbers, long[] countSeq, BiFunction<Long, Long, Boolean> orderFunction) {
        ...
        for (int i = countSeq.length - 1; i >= 0; i--) {
            if (countSeq[i] == currentSeqCount) {
                if (currentSeqCount == count ||
                        orderFunction.apply(numbers[i], numbers[seqArray[currentSeqCount]])) {
